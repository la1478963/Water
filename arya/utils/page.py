...
class Pagination(object):
    ...
    def page_list_html(self):
        html='''<li><select class="form-control" onchange="window.location=this.value">
                 <option>每页显示</option>
                 <option value="{0}?{1}&page_list=20">20</option>
                 <option value="{2}?{3}&page_list=50">50</option>
                 <option value="{4}?{5}&page_list=100">100</option>
            </select></li>'''
        # display_str=self.base_url+'?'+self.query_params
        self.query_params['page']=1
        tpl=html.format(self.base_url,self.query_params.urlencode(),
